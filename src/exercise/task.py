...
def create_task_structure():
    ...
    if os.path.exists(output_file):
        modify = ask(f"The file '{task_id}.json' already exists. Do you want to modify it? (yes/no)",
                     default="no").lower()
        if modify == "yes":
    ...
    while True:
        ...
        if language in task_structure:
            ...
            if version in task_structure[language]:
                overwrite = ask(
                    f"The version '{version}' for language '{language}' already exists. Do you want to overwrite it? (yes/no)",
                    default="no").lower()
                if overwrite != "yes":
        ...
        # Add UUID
        task_structure["UUID"] = str(uuid.uuid4())
